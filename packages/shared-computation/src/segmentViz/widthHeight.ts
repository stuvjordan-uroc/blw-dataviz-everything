...
export function getVizWidth(statsInstanceRef: Statistics, segmentVizConfig: SegmentVizConfig) {
  ...
  for (const groupingQuestion of statsConfig.groupingQuestions) {
    const isX = segmentVizConfig.groupingQuestionKeys.x.find((gqk) => gqk === getQuestionKey(groupingQuestion));
    if (isX) {
  ...
  const maxResponseGroups = Math.max(
    1,
    ...statsConfig.responseQuestions.map((rq) => rq.responseGroups.expanded.length)
  )
  return (
...
export function getVizHeight(statsInstanceRef: Statistics, segmentVizConfig: SegmentVizConfig) {
  let maxSegmentGroupsY = 0;
  const statsConfig = statsInstanceRef.getSessionConfig();
  for (const groupingQuestion of statsConfig.groupingQuestions) {
    const isY = segmentVizConfig.groupingQuestionKeys.y.find((gqk) => gqk === getQuestionKey(groupingQuestion));
    if (isY) {
