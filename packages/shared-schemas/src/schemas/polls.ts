...
//sessions table
export const sessions = pollsSchema.table(
  "sessions",
  {
...
export const questions = pollsSchema.table(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      foreignColumns: [questionsDef.varName, questionsDef.batteryName, questionsDef.subBattery]
    }),
    //ensure no duplicate questions within a session
    unique().on(table.sessionId, table.varName, table.batteryName, table.subBattery)
  ]
)

//respondents table
export const respondents = pollsSchema.table(
  "respondents",
  {
    id: serial("id").primaryKey(),
...
export const responses = pollsSchema.table(
  "responses",
  {
    respondentId: integer().notNull().references(() => respondents.id),
...
export const sessionStatistics = pollsSchema.table(
  "session_statistics",
  {
    sessionId: integer().primaryKey().references(() => sessions.id), //only one row per session!!! Note this means application will have to handle concurrent updates!
